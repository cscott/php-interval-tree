...
final class IntervalTree {
	...
	public function findIntersections( IntervalInterface $interval ): Iterator {
		...
		foreach ( $this->treeSearchInterval( $interval ) as $node ) {
			yield $node->getPair();
		}
	}
	...
	private function insertFixup( Node $insertNode ): void {
		...
		while ( $currentNode !== $this->root && $this->parentOf( $currentNode )->getColor()->isRed() ) {
			$parent = $this->parentOf( $currentNode );
			// The root is black, so a red parent is never the root.
			$grandfather = $this->parentOf( $parent );
			if ( $parent === $grandfather->getLeft() ) {
				...
				if ( $uncleNode->getColor()->isRed() ) {
					...
					$this->parentOf( $currentNode )->setColor( NodeColor::black() );
					$grandfather->setColor( NodeColor::red() );
	...
	private function deleteFixup( Node $fixNode ): void {
		$currentNode = $fixNode;
		while ( $currentNode !== $this->root && $currentNode->getColor()->isBlack() ) {
			// None of the cases below changes the parent of $currentNode.
			$parent = $this->parentOf( $currentNode );
			if ( $currentNode === $parent->getLeft() ) {
				$brotherNode = $parent->getRight();
				...
				if (
					$brotherNode->getLeft()->getColor()->isBlack()
					&& $brotherNode->getRight()->getColor()->isBlack()
				) {
					...
					$brotherNode->setColor( $parent->getColor() );
					$parent->setColor( NodeColor::black() );
	...
	/**
	 * Returns the parent of a node that is known not to be the root.
	 *
	 * @param Node<TPoint, TValue> $node
	...
	private function parentOf( Node $node ): Node {
		...
		if ( $parent === null ) {
			throw new LogicException( 'Interval tree is corrupt: non-root node has no parent' );
		}
		return $parent;
	}
	 ...
	 */
	private function treeSearchInterval( IntervalInterface $interval, ?Node $fromNode = null ): Iterator {
		$fromNode ??= $this->root;
		if ( $fromNode->getLeft() !== $this->nilNode && !$fromNode->notIntersectLeftSubtree( $interval ) ) {
			yield from $this->treeSearchInterval( $interval, $fromNode->getLeft() );
		}
		if ( $fromNode->intersect( $interval ) ) {
			yield $fromNode;
		}
		if ( $fromNode->getRight() !== $this->nilNode && !$fromNode->notIntersectRightSubtree( $interval ) ) {
			yield from $this->treeSearchInterval( $interval, $fromNode->getRight() );
		}
	}
	...
	private function localMinimum( Node $node ): Node {
		$nodeMin = $node;
		while ( $nodeMin->getLeft() !== $this->nilNode ) {
